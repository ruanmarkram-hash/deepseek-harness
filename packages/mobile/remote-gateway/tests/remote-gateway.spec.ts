...
import { RpcId as hostRpcId } from '@deepseek-ai/dsh-host-apiproxy'
...
function api(mux = new Frames<RpcRequest<MuxFrame>>(), listBarrier?: Promise<void>) {
  const list = vi.fn(async (message: RpcRequest<{}>) => {
    await listBarrier
    return { rpcId: message.rpcId, result: { ok: true as const, value: { sessions: [] } } }
  })
  const describe = vi.fn(async (message: RpcRequest<{}>) => ({ rpcId: message.rpcId, result: { ok: true as const, value: { version: 'test' } } }))
  const workspaceList = vi.fn(async (message: RpcRequest<{}>) => ({
    rpcId: message.rpcId,
    result: { ok: true as const, value: { workspaces: [] } },
  }))
...
function gateway(apiProxy: ApiProxy, trusted = true) {
  ...
  const markSeen = vi.fn(async () => ({ id: DEVICE, label: 'Phone' }))
  const devices = {
    get: () => allowed ? {
      id: DEVICE,
      incarnation,
...
describe('RemoteGateway', () => {
  ...
  it('starts a re-enrolled device with no retained idempotency result or old event replay', async () => {
    ...
    await active.receive({
      ...
      deviceId: DEVICE as unknown as RemoteWireId, action: 'device.describe', payload: {},
    })
    ...
    await reconnected.receive({
      version: 3, type: 'device-control', connectionEpoch: 1, requestId: SECOND_REQUEST, idempotencyKey: SECOND_RETRY,
      deviceId: DEVICE as unknown as RemoteWireId, action: 'device.describe', payload: { cursor: 0 },
    })
