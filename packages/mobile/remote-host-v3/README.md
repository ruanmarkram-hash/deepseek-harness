...
Aborting a pending receive removes that reader before a later frame is dispatched, so a replacement reader can receive the next frame. Outbound capacity reservations belong to the provider that created them; another provider cannot release or invalidate them. If a queued send's final fence check throws, the provider closes the private pipe with `REMOTE_HOST_V3_WIRE_WRITE_FAILED` and rejects pending writes. If that check closes the pipe synchronously, no additional descriptor write occurs.
