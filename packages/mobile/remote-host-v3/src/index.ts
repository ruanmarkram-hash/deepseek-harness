...
export class RemoteHostV3Controller implements RemoteHostV3ControllerApi {
  ...
  startWithNative(native: RemoteHostV3NativeProvider): void {
    ...
    const pipeKind: string = native.runtimePipe.kind
    if (pipeKind !== 'inherited-private-pipe') {
  ...
  private requireNative(): RemoteHostV3NativeProvider {
    ...
    const pipeKind: string = this.native.runtimePipe.kind
    if (!absolutePath(this.config.hostAppPath) || this.native.hostAppPath !== this.config.hostAppPath || pipeKind !== 'inherited-private-pipe') {
