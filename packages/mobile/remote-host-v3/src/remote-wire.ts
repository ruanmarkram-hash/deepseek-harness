...
class InheritedWireConnection implements TrustedRemoteConnection {
  ...
  async send(envelope: RemoteWireEnvelope, fence: TrustedRemoteSendFence): Promise<TrustedSendResult> {
    const active = () => this.state === 'open' && fence.active && !fence.abortSignal.aborted
    if (!active()) return { status: 'not-committed' }
    ...
    try {
      ...
      return active() ? { status: 'committed-before-fence' } : { status: 'not-committed' }
    } catch {
...
export class RemoteHostV3InheritedWireProvider implements RemoteHostV3RuntimePipe {
  ...
  async writeConnectionSend(
    ...
    const active = () => fence.active && !fence.abortSignal.aborted
    if (!active()) throw new RemoteHostV3Error('REMOTE_HOST_V3_WIRE_CLOSED', 'Connection send fence is inactive')
    ...
    try {
      ...
      if (!lifetime.active || !active()) throw new RemoteHostV3Error('REMOTE_HOST_V3_WIRE_CLOSED', 'Connection send fence changed before private-pipe commit')
    } finally {
      lifetime.pendingSends -= 1
  ...
  private async handle(record: RemoteHostV3WireRecord): Promise<void> {
    switch (record.kind) {
      ...
      case 'connection.closed': { this.connectionClosed(record); return }
      case 'host.stopping': { this.hostStopping(record); return }
