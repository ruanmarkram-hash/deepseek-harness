...
function identifier(value: unknown): RemoteWireId {
...
export function parseRemoteWireEnvelope(value: unknown): RemoteWireEnvelope {
  ...
  if (value.type === 'request') {
    ...
    return completeEnvelope({
      ...
      requestId: identifier(value.requestId), idempotencyKey: identifier(value.idempotencyKey),
  ...
  if (value.type === 'response') {
    ...
    return completeEnvelope({
      ...
      connectionEpoch: sequence(value.connectionEpoch, 'REMOTE_WIRE_EPOCH_INVALID'),
      requestId: identifier(value.requestId), result: result(value.result),
    })
  }
  if (value.type === 'event') {
    ...
    return completeEnvelope({
      ...
      connectionEpoch: sequence(value.connectionEpoch, 'REMOTE_WIRE_EPOCH_INVALID'),
      cursor: sequence(value.cursor, 'REMOTE_WIRE_CURSOR_INVALID'), eventId: identifier(value.eventId), requestId: identifier(value.requestId),
  ...
  if (value.type === 'approval') {
    ...
    return completeEnvelope({
      ...
      connectionEpoch: sequence(value.connectionEpoch, 'REMOTE_WIRE_EPOCH_INVALID'),
      requestId: identifier(value.requestId), idempotencyKey: identifier(value.idempotencyKey),
      sessionId: identifier(value.sessionId), approvalId: identifier(value.approvalId), outcome: value.outcome,
    })
  }
  ...
  if (value.type === 'device-control') {
    ...
    return completeEnvelope({
      ...
      connectionEpoch: sequence(value.connectionEpoch, 'REMOTE_WIRE_EPOCH_INVALID'),
      requestId: identifier(value.requestId), idempotencyKey: identifier(value.idempotencyKey),
      deviceId: identifier(value.deviceId), action: deviceControl(value.action), payload: payload(value.payload),
    })
  }
