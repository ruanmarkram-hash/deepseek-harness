...
export class RemoteHostIdentity {
  ...
  /** @param handle - Signed native provider handle. @returns Host identity facade. */
  ...
  /** @param payload - Exact handshake transcript bytes. @returns detached signature bytes. */
  sign(payload: Uint8Array): Uint8Array { return new Uint8Array(this.handle.sign(payload)) }

  /** @param remoteAgreementPublicKey - Base64url remote public key. @returns KDF input bytes. */
  deriveSharedSecret(remoteAgreementPublicKey: string): Uint8Array {
