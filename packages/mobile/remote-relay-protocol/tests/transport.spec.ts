...
describe('trusted remote relay v3 transport', () => {
  ...
  it('rejects a stale enrollment incarnation even when the device id and agreement key match', async () => {
    ...
    const connected = connectRemoteRelayDevice({ socket: deviceSocket, identity: stale, host: publicIdentity(host), route: ROUTE, random: random(20) })
  ...
  it('does not expose the Host connection until it validates the device encrypted acknowledgement', async () => {
    ...
    const connecting = connectRemoteRelayDevice({ socket: deviceSocket, identity: device, host: publicIdentity(host), route: ROUTE, random: random(20) })
    let settled = false
  ...
  it('rejects malformed and replay-unsafe opaque frames before any relay may forward them', () => {
    expect(code(() => parseRemoteRelayMessage(JSON.stringify({
      ...
      senderDeviceId: DEVICE_ID, senderEnrollmentId: DEVICE_ENROLLMENT_ID, recipientDeviceId: HOST_ID, recipientEnrollmentId: HOST_ENROLLMENT_ID, sequence: 0,
