...
import type {
  HistoryEntry, MobileContentBlock, MobileSessionEvent, ModelCatalogFailure, ModelCatalogModel, ModelProviderGroup, ModelReasoning,
...
export const sessionIdSchema = z.string().min(1).transform(brandString<SessionId>)

/** MessageId: one brand cast after non-empty string validation. */
export const messageIdSchema = z.string().min(1).transform(brandString<MessageId>)
 ...
 */
export const workspaceIdSchema = z.string().min(1).transform(brandString<WorkspaceId>)
...
}) satisfies z.ZodType<Wire<MobileSessionEvent>>
...
}) satisfies z.ZodType<Wire<SessionProjectionsBlock>>
...
export const contentBlockSchema = z.looseObject({ type: z.string() }) satisfies z.ZodType<MobileContentBlock>
...
/** session.prompt request payload, including the optional browser-local time zone. */
export const sessionPromptRequestSchema = z.object({
  sessionId: sessionIdSchema,
  mode: z.union([z.literal('queue'), z.literal('steer')]),
  content: z.array(promptContentPartSchema),
  clientTimeZone: z.string().optional(),
}) satisfies z.ZodType<Wire<RequestPayload<'session.prompt'>>>
...
export const attachmentIdSchema = z.string().min(1).transform(brandString<AttachmentIdType>)
...
}) satisfies z.ZodType<Wire<ImageAttachmentRef>>
...
}) satisfies z.ZodType<Wire<RequestPayload<'session.updateQueue'>>>
