...
import type { HistoryEntry, MobileContentBlock, SessionProjectionsBlock } from './sessions.ts'
...
export interface SubagentsApi {
   ...
   * The optional browser time zone is validated and logged on that message.
   */
  prompt(
    request: RpcRequest<
      Extract<SubagentAddress, { mode: 'continuable' }> & {
        content: MobileContentBlock[]
