...
/** Released wire event envelope; each consumer narrows the owner-specific data before reading it. */
...
/** Released extensible content envelope; current Host business owners validate its fields. */
export interface MobileContentBlock {
  type: string
  [key: string]: unknown
}
...
export type QueueAction =
  | { kind: 'edit'; content: MobileContentBlock[] }
  | { kind: 'remove' }
  | { kind: 'steer' }
