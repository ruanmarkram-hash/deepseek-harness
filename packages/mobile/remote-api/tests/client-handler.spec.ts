...
import type { ApiProxy, GoalRef, HostFrame, MuxFrame, RpcMessage, RpcRequest, RpcResponse } from '@deepseek-ai/dsh-remote-api'
import { InProcessApiClient, RpcId, toFetchHandler, invokeApiProxyMethod } from '@deepseek-ai/dsh-remote-api'
...
function scriptedApi(overrides: {
  ...
  return {
    sessions: {
      ...
      attachment: r => ok(r, {
        attachment: { attachmentId: AttachmentId('a'), mediaType: 'image/png', bytes: 1, width: 1, height: 1 },
        data: 'AA==',
      }),
    ...
    subagents: {
      ...
      prompt: r => ok(r, { messageId: MessageId('message-1') }),
    ...
    workspace: {
      ...
      create: r => ok(r, { workspace: { workspaceId: workspaceIdSchema.parse('w1'), path: '/t', title: 't', sessionIds: [], createdAt: '0', updatedAt: '0' }, created: true }),
      rename: r => ok(r, { workspace: { workspaceId: workspaceIdSchema.parse('w1'), path: '/t', title: 't', sessionIds: [], createdAt: '0', updatedAt: '0' } }),
      delete: r => ok(r, { deleted: true as const }),
      insertBefore: r => ok(r, { workspaceIds: [r.payload.workspaceId] }),
      insertSessionBefore: r => ok(r, { workspace: { workspaceId: workspaceIdSchema.parse('w1'), path: '/t', title: 't', sessionIds: [], createdAt: '0', updatedAt: '0' } }),
      archiveSession: r => ok(r, { archivedSessionIds: [r.payload.sessionId] }),
    },
    skills: { list: r => ok(r, { skills: [] }), ...overrides.skills },
    plugins: { list: r => ok(r, { plugins: [] }), setEnabled: err, ...overrides.plugins },
    agentPresets: {
...
describe('unary round trip', () => {
  ...
  it('routes workspace rename, delete, and ordering through the wire', async () => {
    const api = scriptedApi()
    const c = client(api)
    const renamed = await c.workspace.rename({ workspaceId: workspaceIdSchema.parse('w1'), title: 'next' })
    expect(renamed.result.ok).toBe(true)
    const blankTitle = await c.workspace.rename({ workspaceId: workspaceIdSchema.parse('w1'), title: '   ' })
    expect(blankTitle.result).toMatchObject({ ok: false, error: { code: 'bad-request' } })
    const deleted = await c.workspace.delete({ workspaceId: workspaceIdSchema.parse('w1') })
    ...
    const workspaceOrder = await c.workspace.insertBefore({
      ...
      beforeWorkspaceId: workspaceIdSchema.parse('w2'),
    })
    expect(workspaceOrder.result).toEqual({ ok: true, value: { workspaceIds: ['w1'] } })
    const anchored = await c.workspace.insertSessionBefore({ workspaceId: workspaceIdSchema.parse('w1'), sessionId: sid('s1'), beforeSessionId: sid('s2') })
    expect(anchored.result.ok).toBe(true)
    const appended = await c.workspace.insertSessionBefore({ workspaceId: workspaceIdSchema.parse('w1'), sessionId: sid('s1') })
  ...
  it('rejects an invalid payload at the handler as 200 + bad-request with issues', async () => {
    const api = scriptedApi()
    const response = await invokeApiProxyMethod(api, 'session.history', { rpcId: RpcId('invalid'), payload: { sessionId: 123 } }, new AbortController().signal)
  ...
  it('round-trips subagent.interrupt and rejects a one-shot or incomplete address', async () => {
    ...
    const oneShot = await invokeApiProxyMethod(api, 'subagent.interrupt', { rpcId: RpcId('one-shot'), payload: {
    ...
    const incomplete = await invokeApiProxyMethod(api, 'subagent.interrupt', { rpcId: RpcId('incomplete'), payload: {
  ...
  it('throws on an S→C ok value that fails the method value schema (second-level parse)', async () => {
    ...
    const malformed = new InProcessApiClient({
      async fetch(input, init) {
        const request = new Request(input, init)
        const body = await request.json() as { rpcId: string }
        return Response.json({ type: 'server-response', rpcId: body.rpcId, result: { ok: true, value: { items: 'not-an-array' } } })
      },
    })
    await expect(malformed.sessions.list({})).rejects.toThrow()
  })
})
...
describe('workspace domain round trip', () => {
  it('routes both workspace methods through their handler rows and value schemas', async () => {
    ...
    const archivedResponse = await c.workspace.archiveSession({ sessionId: SessionId('s-arch') })
  ...
  it('rejects a pathless create payload at the handler schema', async () => {
    const response = await invokeApiProxyMethod(scriptedApi(), 'workspace.create', { rpcId: RpcId('pathless'), payload: {} }, new AbortController().signal)
    expect(response.result.ok).toBe(false)
