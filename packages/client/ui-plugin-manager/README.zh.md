...
使用 Web 侧栏的**插件**入口管理 profile 的组合包或已签名 Host 的内置插件。在受管 profile 中，可以启停组合包及其行，在 Host 读出 spec 后安装组合包，并在确认后卸载。在已签名 Host 中，可以启停可选内置插件；必需服务保持锁定。注册了配置页的插件在这里编辑；设置里保留只读的插件列表。
...
在侧栏选择**插件**。在受管 profile 中，页面通过 `api-remotes` 读取清单与组合包。**官方**排在前面，列出安装随附、供开启的组合包——开启前保持关闭、没有卸载、属于实验性功能的带**实验性**标签——其后是注册了配置页的官方插件；**已安装**列出 profile 持有的组合包。卡片按名称排序，启停组合包不会挪动它的卡片。没有组合包 patch 的依赖不是插件，除非 profile 选中了它才会带异常标签列出。全局配置仍在设置的**插件**分区中编辑。

### 已签名 Host 插件

清单报告没有受管 profile 时，页面向已签名 Host 请求插件列表，显示已签名内置行及当前启用状态。可选行可以启停，Host 应用更改后页面会刷新；必需行保持可见，开关锁定并附有说明。已签名 Host 页面没有添加或卸载操作。读取或切换失败时显示错误，并提供重试或刷新。此页面不会向已签名 Host 安装可执行包。
