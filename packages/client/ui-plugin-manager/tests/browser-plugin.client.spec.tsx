...
async function bench() {
  ...
  return { ctx, slots: ctx.get('slots') as SlotRegistry, locale, list, hostList, remote, selectPanel, panelInfo }
}
...
describe('ui-plugin-manager browser plugin', () => {
  ...
  it('declares only the services the page and its Remote methods use', () => {
    expect(inject).toEqual(['slots', 'locale', 'remote', 'remote.pluginManager', 'remote.pluginInventory', 'remote.pluginRegistryProbe', 'remote.hostPlugins', 'configForms', 'layout'])
  })
