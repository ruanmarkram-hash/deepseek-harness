...
The workspace security descriptor edits are granted standing — `dispose()` leaves them, because they are the cross-instance reuse cache — while the distinct temp SID is granted revocably. Each root grant combines the capability-SID allow ACE, the ambient-delete deny, and the Low no-write-up label. Before spawn, existing subdirectories also receive an explicit inheritable delete deny; directory reparse points are not traversed, and any failed inspection or ACL edit aborts the grant. The server-side counterpart is the `AclWriteGrant` class: `add(path, standing)` per directory, and `dispose()` revokes the revocable paths and frees the SIDs.
...
- **Grant materialization scans existing directories** — the root's inheritable ACEs propagate eagerly, and each existing subdirectory receives an explicit delete deny before spawn. The root's exact-ACE skip avoids repeating its propagation, but every provision still scans the tree and repairs missing descendant denies; large workspaces can take substantial time.
