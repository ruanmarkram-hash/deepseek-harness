...
In-process carriers use `wireRpc(endpoint, payload, signal, peer?)` for unary calls and `$events/result` responses, sharing Connection dispatch validation, result envelopes, and pending event ownership. Omitting `peer` selects the operator. Gateway resolves the optional `fd199DesktopWriteFence` service for every unary operation, including direct `invoke()` and Client event responses, and holds it through settlement. An absent fence permits dispatch; a closed or malformed fence rejects it. Ordinary streams and the forwarded `$events` stream remain unfenced.
