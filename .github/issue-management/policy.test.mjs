...
test('keeps trusted preflight before token minting and required policy unconditional within its configured repository', () => {
  ...
  const jobConditions = job.slice(0, job.indexOf('    steps:')).split('\n').filter(line => line.startsWith('    if:'))
  assert.deepEqual(jobConditions, ["    if: ${{ github.repository == 'deepseek-harness/deepseek-harness' }}"])
  const config = JSON.parse(readFileSync(new URL('./config.json', import.meta.url), 'utf8'))
  assert.equal(`${config.organization}/${config.repository}`, 'deepseek-harness/deepseek-harness')
