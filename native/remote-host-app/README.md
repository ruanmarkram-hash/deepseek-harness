...
The native epoch ledger permits one live Host route owner, commits an epoch only after the authenticated handshake, and reconciles a lost receipt without skipping the next epoch. A phone network interruption can reconnect through the retained route at the Host-issued next epoch. After a clean Host restart, starting the local runtime still leaves phone activation explicit. A failed activation retires its seeded child; retry with **Start hosted runtime**, then **Activate paired phone**. Pairing credentials, the signed journal and native epochs remain intact. Stop cancels pending activation and waits for in-flight lifecycle cleanup before permitting another owner.
