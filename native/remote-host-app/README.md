...
`CFBundleIconFile` selects the `DeepSeek.icns` resource, copied from the `DeepSeek-HOST.icns` variant into the Host before signing. The whale carries a small `HOST` label at launcher sizes. The [Desktop icon packaging reference](../../apps/desktop/README.md) owns artwork generation and verification.
...
After pairing, **Start hosted runtime** launches the sealed hosted child and recovers an eligible signed FD199 journal without constructing a phone session or opening a relay socket. The hosted child receives relay records on fixed descriptor 198 and authority handoff records on fixed descriptor 199; it receives neither the Host token nor private agreement material. **Activate paired phone** completes fresh signed FD199 ownership or resumes already-activated ownership before the native Host opens the authenticated V3 relay WebSocket. The browser and phone then use the same hosted runtime.
...
Explicit phone activation waits up to 120 seconds for the first paired-phone frame, allowing device-owner authentication before the cryptographic handshake begins. The exact first frame receives the normal route, enrollment and epoch validation; malformed input is not retried. Cryptographic flights retain their 10-second deadlines. Timeout errors distinguish waiting for the phone from processing hello or awaiting ready, ack or confirm without exposing credentials. Stop and durable revocation prevent a late first frame from creating a transport.

Rejected input reports a fixed phase and category: malformed flight, route tuple mismatch, epoch mismatch, invalid key material, or an exact allowlisted relay control reason. Relay reports always terminate the connection and never authorize local revocation, epoch changes or credential replacement. Unknown, oversized, duplicate-key or extended control envelopes remain malformed input; raw frames and arbitrary server text are never displayed.

The native epoch ledger permits one live Host route owner, commits an epoch only after the authenticated handshake, and reconciles a lost receipt without skipping the next epoch. After an established phone transport ends, the local Web runtime remains available until an explicit **Activate paired phone** retry. That action waits for the ended transport's cleanup, stops its seeded child, and starts a fresh child and phone session through the retained route at the Host-issued next epoch. An active phone session rejects overlapping activation. After a clean Host restart, starting the local runtime still leaves phone activation explicit.

A failed activation or replacement startup retires its seeded child; retry with **Start hosted runtime**, then **Activate paired phone**. Pairing credentials, the signed journal and native epochs remain intact. Stop cancels pending activation and waits for in-flight lifecycle cleanup before permitting another owner. The [recovery decision](../../.agents/notes/implemented/bug-fix/2026-09-23-hosted-phone-session-recovery.md) records the one-shot enrollment and cleanup requirements.
...
FD199 version 2 incrementally verifies complete logical histories with acknowledged 256 KiB chunks, without retaining file contents in native memory. A complete export retains the 128 MiB aggregate and 8,192-file limits. New journals and signed proofs use version 3; existing version 2 records retain their original proof bytes, admission limits and activation version. Transfer timeout or interrupted verification never promotes ownership. The [handoff reference](docs/hosted-runtime-runbook.md#fd199-streamed-history-handoff) owns the protocol and recovery details.

An authenticated `connection.open` carries the full eight-field connection metadata. Subsequent `connection.frame` and child `connection.send` records carry only `{ connectionId }`; a child `connection.close` carries exactly `{ connectionId, reason }` with a fixed gateway close reason. The native frame pump derives this reference from the open record and matches parsed connection IDs, rejecting extra or duplicate fields without depending on JSON ordering or escaping. A stale close cannot stop the current connection.
...
The [Host-attached Desktop shell](desktop-shell/README.md) opens this Host's existing authenticated Web runtime without starting another owner. It is packaged separately from the upstream runtime-owning Desktop application.
