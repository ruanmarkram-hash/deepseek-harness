...
import { RpcId } from '../../../packages/mobile/remote-api/src/api/rpc.ts'

const SESSION = 'sealed_session_0001'

test('unlinked sealed API capabilities fail closed with no ambient service fallback', async () => {
  const api = unavailableApi()
  await assert.rejects(api.host.describe({ rpcId: RpcId('unavailable'), payload: {} }), /capability is unavailable/)
  await assert.rejects(api.settings.describe({ rpcId: RpcId('unavailable'), payload: {} }), /capability is unavailable/)
})
