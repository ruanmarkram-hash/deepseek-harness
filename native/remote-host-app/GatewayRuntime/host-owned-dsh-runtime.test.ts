...
import type { RemoteDeviceId, RemoteDeviceIncarnation, RemoteDeviceRecord } from '../../../packages/mobile/remote-devices/src/types.ts'
...
function services(): SealedHostDshServices {
  ...
  const remoteDevices = directoryFixture({
    get: (deviceId: RemoteDeviceId) => deviceId === DEVICE ? device : undefined,
    markSeen: async () => device,
  })
  const list: ApiProxy['sessions']['list'] = async request => ({ rpcId: request.rpcId, result: { ok: true, value: { items: [] } } })
  const empty = async function* (): AsyncIterable<never> {}
  const apiProxy = unavailableApi()
  apiProxy.sessions.list = list
  apiProxy.events = { mux: empty, host: empty }
  return {
    apiProxy,
    remoteDevices,
