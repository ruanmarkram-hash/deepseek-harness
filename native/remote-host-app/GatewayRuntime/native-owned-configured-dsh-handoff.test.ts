...
import type { RemoteDeviceDirectory, RemoteDeviceId } from '../../../packages/mobile/remote-devices/src/types.ts'
...
function hostServices(): Omit<SealedHostDshServices, 'apiProxy'> {
  ...
  return {
    remoteDevices: { get: () => undefined } as unknown as RemoteDeviceDirectory,
    routeAllocator,
...
test('only native may atomically consume activation and retain the exact desktop graph for fixed remote work', async () => {
  ...
  const lease = await startActivatedConfiguredDshHostRuntime({
    ...
    async startActivatedFixedRemoteGateway(received) {
      ...
      const remote = await desktopApi.sessions.list({ rpcId: 'remote-list' } as never)
      assert.equal((remote as { result: { value: { sessions: Array<{ id: string; model: unknown }> } } }).result.value.sessions[0]?.id, session.id)
      assert.deepEqual((remote as { result: { value: { sessions: Array<{ model: unknown }> } } }).result.value.sessions[0]?.model, session.model)
      const iterator = desktopApi.events.mux({} as never)[Symbol.asyncIterator]()
