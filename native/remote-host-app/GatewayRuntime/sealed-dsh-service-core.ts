...
import type { ClientResponse, RpcReceipt } from '../../../packages/host/apiproxy/src/api/rpc.ts'
import type { MuxFrame } from '../../../packages/host/apiproxy/src/api/events.ts'
...
class FixedCoreRuntime {
  ...
  api(): ApiProxy {
    const ok = <T>(rpcId: string, value: T) => ({ rpcId, result: { ok: true as const, value } })
    const missing = <T>(rpcId: string, sessionId: string) => ({ rpcId, result: { ok: false as const, error: { code: 'session-not-found' as const, message: 'session not found', details: { sessionId } } } }) as T
    return {
      sessions: {
        list: async request => ok(request.rpcId, { items: Object.values(this.state.sessions).map(session => ({ sessionId: session.sessionId, updatedAt: session.updatedAt, running: false, blank: session.events.length === 0 })) }),
        ...
        history: async request => {
          const payload = request.payload as { readonly sessionId: string; readonly beforeSeq?: number; readonly maxMessages?: number }
        ...
        prompt: async request => this.prompt(request as never, ok, missing),
      },
      events: {
  ...
  private async prompt(request: { readonly rpcId: string; readonly payload: { readonly sessionId: string; readonly content: readonly { readonly type: string; readonly text?: string }[] } }, ok: <T>(rpcId: string, value: T) => unknown, missing: <T>(rpcId: string, sessionId: string) => T): Promise<unknown> {
    ...
    const text = request.payload.content.map(part => part.text!).join('')
    ...
    return ok(request.rpcId, { accepted: true })
  }
