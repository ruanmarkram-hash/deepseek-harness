...
cmp -s "$build/build-attestation.json" "$root/third_party/libsodium/prebuilt/macos-arm64/PREBUILT.json" || { print -u2 'verified libsodium build attestation does not match the tracked prebuilt'; exit 65; }
