...
async function spawnHostedChild(
  ...
  const fillers: string[] = Array.from({ length: 195 }, () => 'ignore')
  ...
  const childStdio = ['ignore', 'pipe', 'pipe', ...fillers, relayChild, authorityChild] as unknown as import('node:child_process').StdioOptions
  const sourceLaunch = packagedNode === undefined
  let hostedArgs: string[] | undefined
  if (!sourceLaunch && packagedWebArgs !== undefined) {
    try {
      const decoded = JSON.parse(packagedWebArgs) as unknown
  ...
  const tsxLoader = sourceLaunch ? import.meta.resolve('tsx') : undefined
...
async function api<T>(base: string, method: string, payload: unknown): Promise<T> {
  rpcCounter += 1
  const response = await fetch(`${base}/api/${method}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ type: 'client-request', rpcId: `desktop-${String(rpcCounter)}`, method, payload }),
  })
...
describe('hosted FD199 mirror across real dsh web children', () => {
  it('mirrors a desktop-created session identically to the v3 remote path after an attested activation', { timeout: 2_700_000, retry: 0 }, async () => {
    ...
    const home = configuredHome ?? await mkdtemp(join(tmpdir(), 'dsh-fd199-mirror-home-'))
    ...
    try {
      ...
      const registry = JSON.parse(registryText) as { url: string; pid: number }
      ...
      const created = await api<{ sessionId: string }>(base, 'session.create', {})
      ...
      const adoptedRegistry = JSON.parse(adoptedRegistryText) as { url: string; pid: number }
      ...
      // 6. Mirror identity: the same ApiProxy answers both carriers with the
      // exact same sessions in the exact same shape.
      const desktopList = await api<{ items: Array<{ sessionId: string }> }>(adoptedBase, 'session.list', {})
      const desktopIds = desktopList.items.map(session => session.sessionId)
      expect(desktopIds).toContain(created.sessionId)
      const remoteIds = remoteEnvelope.result.value.items.map(session => String(session.sessionId))
