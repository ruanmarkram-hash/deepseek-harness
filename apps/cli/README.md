...
[Web runtime discovery](src/web-runtime-registry.ts) keeps the noncredential `runtime/web.json` record separate from `runtime/web-bootstrap.json`, which carries the current browser launch capability. Bootstrap publication requires matching process, start instant, and loopback origin; same-user ownership; a non-writable-by-others Harness home; a `0700` runtime directory; and `0600` regular files. The bounded, no-symlink read rejects unsafe records, and retiring publishers remove only their own capability. The Desktop shell consumes this private file to perform Connection's existing root token-to-cookie exchange, not to bypass browser authentication.
