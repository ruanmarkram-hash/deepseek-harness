...
EAS 安装后步骤和本地导出准备会编译协议项目，将共用值辅助函数打包到声明的运行时入口，然后在 Metro 运行前打包 wire 和 relay 包。仅有 `lib/types` 下的编译器输出无法满足这些包的运行时导入。此准备过程不会更改配对身份或协议格式。
...
会话抽屉通过已认证连接打开 Host 插件列表。内置插件与 Host 下载的插件一同显示；可选插件可以启用或禁用，必需插件显示锁定原因。手机仅在 Host 返回已提交状态后更新开关。手机不能安装、下载或移除可执行软件包；插件变更使用实时连接已完成的所有者认证。
