...
describe('NativeMobileRemoteStateStore', () => {
  it('persists only the invitation epoch and advances it only after an authenticated receipt', async () => {
    ...
    const reopened = new NativeMobileRemoteStateStore(native)
    await expect(reopened.nextConnectionEpoch({ ...config, connectionEpoch: 2 }, 2, new AbortController().signal)).resolves.toBe(2)
  })
