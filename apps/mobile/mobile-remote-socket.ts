...
export class ReactNativeMobileRemoteSocketFactory implements MobileRemoteSocketFactory {
  constructor(
    private readonly webSocket: MobileNativeWebSocketFactory = nativeWebSocket,
  ) {}
...
function nativeWebSocket(url: string, protocols: readonly string[]): MobileNativeWebSocket {
  const socket = new WebSocket(url, [...protocols])
  const carrier: MobileNativeWebSocket = {
    get readyState() { return socket.readyState },
    onclose: null, onerror: null, onmessage: null, onopen: null,
    close(code, reason) { socket.close(code, reason) },
    send(data) { socket.send(data) },
  }
  socket.onclose = event => carrier.onclose?.({ code: event.code })
  socket.onerror = () => carrier.onerror?.()
  socket.onmessage = event => carrier.onmessage?.({ data: event.data })
  socket.onopen = () => carrier.onopen?.()
  return carrier
}
