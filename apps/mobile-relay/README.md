...
An authenticated Host socket may wait 125 seconds before the phone's first valid hello: the native Host's 120-second rendezvous budget plus five seconds of relay scheduling margin. An unbound device socket still expires after 30 seconds. The first accepted hello starts the unchanged 30-second cryptographic handshake budget, including a Host that has not sent welcome yet. Alarms, pre-forward expiry checks and expired-owner replacement use the same role-specific deadlines; valid transitions recompute the earliest alarm. These waits do not relax authentication, exclusive peer ownership, revocation, or epoch validation.
