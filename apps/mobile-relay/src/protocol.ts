...
/** Authenticated control messages carried outside the encrypted frame payload. */
export type RelayControl =
  | { readonly type: 'desktop-hello'; readonly pairingId: string; readonly desktopDeviceId: string }
  | { readonly type: 'mobile-request'; readonly pairingId: string; readonly mobileDeviceId: string; readonly capabilities: readonly MobilePairingCapability[] }
  | { readonly type: 'desktop-accept'; readonly pairingId: string; readonly mobileDeviceId: string }
...
export function parseConnectionToken(value: string | null): ConnectionToken | undefined {
  ...
  if (protocols.length !== 2 || protocols[0] !== 'dsh-pairing-v1') return undefined
  const tokenProtocol = protocols[1]
  if (tokenProtocol === undefined) return undefined
  for (const [peer, prefix] of [['desktop', 'dsh-desktop.'], ['mobile', 'dsh-mobile.']] as const) {
 ...
 * @returns The bounded desktop identity and pairing expiry.
 */
export function parsePairingCreation(value: string, nowMs = Date.now()): PairingCreation | undefined {
  if (value.length > MAX_CONTROL_MESSAGE_BYTES || !Number.isSafeInteger(nowMs)) return undefined
  let parsed: unknown
  try {
  ...
  if (!isRecord(parsed) || !hasExactKeys(parsed, ['version', 'desktopDeviceId', 'mobileRelayToken', 'expiresAt'])) return undefined
  if (parsed.version !== PAIRING_PROTOCOL_VERSION) return undefined
  const desktopDeviceId = readPublicId(parsed.desktopDeviceId)
  const mobileRelayToken = typeof parsed.mobileRelayToken === 'string' && RELAY_TOKEN.test(parsed.mobileRelayToken)
    ? parsed.mobileRelayToken
    : undefined
  const expiresAt = readFutureExpiry(parsed.expiresAt, nowMs)
  return desktopDeviceId === undefined || mobileRelayToken === undefined || expiresAt === undefined
    ? undefined
    : { desktopDeviceId, mobileRelayToken, expiresAt }
}

function parseControl(value: Record<string, unknown>): RelayControl | undefined {
  if (value.version !== PAIRING_PROTOCOL_VERSION || typeof value.type !== 'string') return undefined
  switch (value.type) {
    case 'desktop-hello': {
      if (!hasExactKeys(value, ['type', 'version', 'pairingId', 'desktopDeviceId'])) return undefined
    ...
    case 'mobile-request': {
      if (!hasExactKeys(value, ['type', 'version', 'pairingId', 'mobileDeviceId', 'capabilities'])) return undefined
      const mobileDeviceId = readPublicId(value.mobileDeviceId)
      const pairingId = readPublicId(value.pairingId)
      if (mobileDeviceId === undefined || pairingId === undefined) return undefined
      try {
        return { type: 'mobile-request', pairingId, mobileDeviceId, capabilities: validateMobileCapabilities(value.capabilities) }
      } catch {
        return undefined
      }
    }
    case 'desktop-accept': {
      if (!hasExactKeys(value, ['type', 'version', 'pairingId', 'mobileDeviceId'])) return undefined
      const mobileDeviceId = readPublicId(value.mobileDeviceId)
      const pairingId = readPublicId(value.pairingId)
      return mobileDeviceId === undefined || pairingId === undefined
        ? undefined
        : { type: 'desktop-accept', pairingId, mobileDeviceId }
    }
    case 'desktop-revoke':
 ...
 * small control vocabulary only establishes or ends a pairing.
 *
