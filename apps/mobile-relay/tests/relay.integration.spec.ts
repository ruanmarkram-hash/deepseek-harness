...
import type { Env, RemoteRoute, V3Pairing } from '../src/index.ts'
...
function internalRequest(request: Request, action: string, id: string, family: 'remote' | 'v3-pairing'): Request {
  const headers = new Headers(request.headers)
  headers.set(`x-dsh-${family}-action`, action)
  headers.set(family === 'remote' ? 'x-dsh-remote-route-id' : 'x-dsh-v3-pairing-id', id)
  return new Request(request, { headers })
}

function pairingFixture(instance: V3Pairing) {
  const invoke = (request: Request, id: string, action: string) => instance.fetch(internalRequest(request, action, id, 'v3-pairing'))
  return {
    submitOffer: (request: Request, id: string) => invoke(request, id, 'submit-offer'),
    publishInvitation: (request: Request, id: string) => invoke(request, id, 'publish-invitation'),
    acknowledgeInvitation: (request: Request, id: string) => invoke(request, id, 'acknowledge-invitation'),
  }
}

function routeFixture(instance: RemoteRoute) {
  const invoke = (request: Request, id: string, action: string) => instance.fetch(internalRequest(request, action, id, 'remote'))
  return {
    create: (request: Request, id: string) => invoke(request, id, 'create'),
    openConnection: (request: Request, id: string) => invoke(request, id, 'connect'),
    rotate: (request: Request, id: string) => invoke(request, id, 'rotate'),
    revoke: (request: Request, id: string) => invoke(request, id, 'revoke'),
    alarm: () => instance.alarm(),
    webSocketMessage: (socket: WebSocket, raw: string) => instance.webSocketMessage(socket, raw),
    webSocketClose: (socket: WebSocket) => instance.webSocketClose(socket),
    // These race tests deliberately suspend the metadata read between awaits.
    // Check the reflected method before replacing it, without widening the class's public API.
    get metadata(): () => Promise<Record<string, unknown> | undefined> {
      const method: unknown = Reflect.get(instance, 'metadata')
      if (typeof method !== 'function') throw new Error('Route metadata method unavailable')
      return async () => {
        const value: unknown = await Reflect.apply(method, instance, [])
        if (value === undefined) return undefined
        if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new Error('Invalid route metadata')
        return Object.fromEntries(Object.entries(value))
      }
    },
    set metadata(method: () => Promise<unknown>) {
      if (!Reflect.set(instance, 'metadata', method)) throw new Error('Cannot instrument route metadata')
    },
  }
}

function namespaceWithFetch<S extends { fetch(request: Request): Promise<Response> }, N extends { getByName(name: string): S }>(
  namespace: N, stub: S, fetch: (request: Request) => Promise<Response>,
): N {
  const wrapped = new Proxy(stub, { get(target, key, receiver) {
    return key === 'fetch' ? fetch : Reflect.get(target, key, receiver)
  } })
  return new Proxy(namespace, { get(target, key, receiver) {
    return key === 'getByName' ? () => wrapped : Reflect.get(target, key, receiver)
  } })
}
...
describe('V3 anywhere pairing rendezvous', () => {
  it('cancels an oversized chunked public creation body at the fixed byte bound', async () => {
    ...
    const request = new Request(anywhereUrl(), {
      method: 'POST',
      headers: { authorization: 'Bearer ' + (env).V3_PROVISIONING_TOKEN },
    ...
    const response = await routeV3Pairing(request, env)
  ...
  it('rejects a wrong pairing code without consuming the public offer body', async () => {
    const relayEnv = env
    await exports.default.fetch(anywhereUrl(), {
  ...
  it('rechecks pairing state when invitation publication races with deletion after preflight', async () => {
    ...
    const racingEnv: Env = {
      ...relayEnv,
      V3_PAIRINGS: namespaceWithFetch(relayEnv.V3_PAIRINGS, pairing, async (request) => {
  ...
  it('acknowledges an invitation through credential preflight and reauthentication', async () => {
    ...
    expect((await exports.default.fetch(anywhereUrl('/invitation'), {
  ...
  it('exchanges only a public offer and a phone-safe invitation using one short-lived QR/code capability', async () => {
    ...
    const created = await exports.default.fetch(anywhereUrl(), {
      method: 'POST',
      headers: { authorization: 'Bearer ' + (env).V3_PROVISIONING_TOKEN },
    ...
    const submitted = await exports.default.fetch(anywhereUrl('/offer'), {
    ...
    const receivedOffer = await exports.default.fetch(anywhereUrl('/offer'), { headers: { authorization: 'Bearer ' + ANYWHERE_HOST_TOKEN } })
    ...
    const published = await exports.default.fetch(anywhereUrl('/invitation'), {
    ...
    const fetched = await exports.default.fetch(anywhereUrl('/invitation'), { headers: { 'x-dsh-pairing-code': ANYWHERE_PAIRING_CODE } })
    expect(await fetched.json()).toEqual(invitation)

    expect((await exports.default.fetch(anywhereUrl('/invitation'), { headers: { 'x-dsh-pairing-code': ANYWHERE_PAIRING_CODE } })).status).toBe(200)
    expect((await exports.default.fetch(anywhereUrl('/invitation'), { method: 'POST', headers: { 'x-dsh-pairing-code': ANYWHERE_PAIRING_CODE } })).status).toBe(204)
    expect((await exports.default.fetch(anywhereUrl('/invitation'), { headers: { 'x-dsh-pairing-code': ANYWHERE_PAIRING_CODE } })).status).toBe(404)
  })
  ...
  it('accepts exactly one of two concurrent valid offers', async () => {
    ...
    const result = await runInDurableObject(pairing, async (instance, state) => {
      const subject = pairingFixture(instance)
  ...
  it('retains neither pairing capability nor a usable device route credential', async () => {
    ...
    await exports.default.fetch(anywhereUrl('/offer'), { method: 'POST', headers: { 'x-dsh-pairing-code': ANYWHERE_PAIRING_CODE }, body: JSON.stringify({ deviceId: DEVICE_ID, label: 'Ruan’s iPhone', signingPublicKey: MOBILE_PUBLIC_KEY, agreementPublicKey: DESKTOP_PUBLIC_KEY }) })
    const invitation = anywhereInvitation()
    expect((await exports.default.fetch(anywhereUrl('/invitation'), { method: 'POST', headers: { authorization: 'Bearer ' + ANYWHERE_HOST_TOKEN }, body: JSON.stringify(invitation) })).status).toBe(204)
...
async function open(peer: 'desktop' | 'mobile'): Promise<WebSocket> {
  const response = await exports.default.fetch(connectRequest(peer))
...
async function openRemote(peer: 'host' | 'device'): Promise<WebSocket> {
  const response = await exports.default.fetch(remoteConnectRequest(peer))
  expect(response.status).toBe(101)
  const socket = response.webSocket
  expect(socket).not.toBeNull()
  socket!.accept()
...
async function createAnywherePairing(relayEnv: Env): Promise<void> {
  const response = await exports.default.fetch(anywhereUrl(), {
    method: 'POST', headers: { authorization: 'Bearer ' + relayEnv.V3_PROVISIONING_TOKEN },
    body: JSON.stringify({ code: ANYWHERE_PAIRING_CODE, hostToken: ANYWHERE_HOST_TOKEN, expiresAt: Date.now() + 60_000 }),
  })
...
async function submitAnywhereOffer(relayEnv: Env): Promise<void> {
  await createAnywherePairing(relayEnv)
  expect((await exports.default.fetch(anywhereUrl('/offer'), {
    method: 'POST', headers: { 'x-dsh-pairing-code': ANYWHERE_PAIRING_CODE },
    body: JSON.stringify({
      deviceId: DEVICE_ID,
      label: 'Test iPhone',
...
describe('mobile pairing relay v2', () => {
  it('stores only verifiers and bounded routing state for a v2 desktop creation', async () => {
    ...
    const created = await exports.default.fetch(pairingUrl(), {
      method: 'POST',
  ...
  it('forwards opaque init and accept proofs, gates frames, and erases state on desktop close', async () => {
    const expiresAt = Date.now() + 60_000
    await exports.default.fetch(pairingUrl(), {
      method: 'POST',
      headers: creationHeaders(),
      body: creationBody(expiresAt),
    })
...
describe('trusted remote relay v3', () => {
  ...
  it('keeps the authenticated idle Host through 30 seconds and expires it at 125 seconds', async () => {
    ...
    await runInDurableObject(route, async (instance, state) => {
      const subject = routeFixture(instance)
      now.mockReturnValue(startedAt)
  ...
  it('does not let an old-token revoke delete a concurrently rotated route', async () => {
    ...
    expect((await exports.default.fetch(remoteConnectRequest('host', NEXT_HOST_TOKEN))).status).toBe(101)
  })
  ...
  it('reports an already deleted route as absent when the Host retries revoke recovery', async () => {
    ...
    const created = await exports.default.fetch(new Request(routeUrl, {
      method: 'POST',
    ...
    const revoke = (credential = HOST_TOKEN): Promise<Response> => exports.default.fetch(new Request(routeUrl, {
      method: 'DELETE',
  ...
  it('rejects a streamed route-rotation body once it crosses the fixed control bound', async () => {
    ...
    const response = await exports.default.fetch(new Request('https://relay.test/v3/routes/' + ROUTE_ID + '/rotate', {
      method: 'POST',
  ...
  it('rechecks the Host credential when rotation races with a successful intervening rotation', async () => {
    ...
    const racingEnv: Env = {
      ...relayEnv,
      REMOTE_ROUTES: namespaceWithFetch(relayEnv.REMOTE_ROUTES, route, async (request) => {
  ...
  it('allows exactly one concurrent rotation to commit from the same old credential snapshot', async () => {
    ...
    const connectionStatuses = await Promise.all([
      exports.default.fetch(remoteConnectRequest('host', NEXT_HOST_TOKEN)),
      exports.default.fetch(remoteConnectRequest('host', alternateHostToken)),
    ])
  ...
  it('rotates both route credentials, changes generation, and immediately rejects old connections', async () => {
    ...
    expect(await hostClosed).toMatchObject({ code: 4403 })
    expect(await deviceClosed).toMatchObject({ code: 4403 })
    expect((await exports.default.fetch(remoteConnectRequest('host'))).status).toBe(401)
    expect((await exports.default.fetch(remoteConnectRequest('host', NEXT_HOST_TOKEN))).status).toBe(101)
  })
