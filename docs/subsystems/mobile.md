...
English | [中文](mobile.zh.md)

The `packages/mobile` group defines platform-neutral protocols and Host services for trusted remote access. The Host keeps public device enrollments in durable storage, accepts only provider-authenticated connections, and emits public lifecycle and audit records without exposing request payloads, credentials, shared secrets, or private keys. These packages add no model-facing context.

Source: [`packages/mobile/remote-devices/src/types.ts`](../../packages/mobile/remote-devices/src/types.ts) · [`packages/mobile/remote-gateway/src/types.ts`](../../packages/mobile/remote-gateway/src/types.ts)

## Trusted-device changes

`remote-devices/changed` emits after a durable enrollment, presence update, or revocation. Enrollment and presence changes carry the current public device record; revocation carries only the opaque device id. Consumers switch on `type`.
...
`remote-gateway/audit` reports the authenticated device and route, the decided operation, its outcome, a stable reason, and optional request correlation. The entry never copies the remote request payload or Host secrets.

```ts type-equiv
/** Post-decision audit record carrying authenticated requester provenance. */
...
Source: [`packages/mobile/remote-devices/src/index.ts:165`](../../packages/mobile/remote-devices/src/index.ts)
...
Source: [`packages/mobile/remote-host-identity/src/controller.ts:14`](../../packages/mobile/remote-host-identity/src/controller.ts)
...
Types: [RpcMethodMap](typert.md)

Source: [`packages/mobile/remote-gateway/src/index.ts:195`](../../packages/mobile/remote-gateway/src/index.ts)
...
Source: [`packages/mobile/remote-host-identity/src/identity.ts:17`](../../packages/mobile/remote-host-identity/src/identity.ts)
...
Source: [`packages/mobile/remote-host-v3/src/index.ts:277`](../../packages/mobile/remote-host-v3/src/index.ts)
...
Source: [`packages/mobile/remote-devices/src/index.ts:94`](../../packages/mobile/remote-devices/src/index.ts)
...
A trusted remote operation reached a Host gateway decision point. The entry contains authenticated provenance and never copies a payload.
...
Source: [`packages/mobile/remote-gateway/src/index.ts:87`](../../packages/mobile/remote-gateway/src/index.ts)
