...
[relay](../../../../apps/mobile-relay/README.zh.md) 允许已认证且未绑定的 Host 等待 125 秒，即[原生会合](2026-09-23-host-paired-phone-rendezvous.zh.md)预算加五秒 relay 调度余量。未绑定设备保留 30 秒限制。hello 之后，未绑定 Host 使用握手的 30 秒截止时间，而非较早的连接时间戳。alarm 调度、过期和过期连接所有者驱逐共享此计算。握手状态转换会重新调度最早的 alarm，但不延长握手开始时间。
