...
This record describes the legacy v2 bridge retained in the installed Desktop wrapper; its source remains in Git history. The maintained [`apps/desktop`](../../../../apps/desktop/README.md) implements the upstream shell and no longer contains these legacy modules. Current remote pairing belongs to the [signed Host](../../../../native/remote-host-app/README.md).

The legacy `mobile-pairing` module is an Electron-main-process-only creator for one Cloudflare pairing. Its constructor accepts a build-owned HTTPS relay origin and no renderer-provided value. Each creation generates separate 32-byte base64url pairing and desktop ids plus separate desktop and mobile relay credentials, expires after four minutes, and posts the exact version-two creation body with the desktop credential in the authorization header.
