...
该 fork 在 `apps/` 下包含两个私有产品工作区。`apps/desktop` 启动本地 DSH Web 运行时，并在禁用 Node integration 的 Electron 窗口中只渲染其 loopback URL。`apps/mobile` 是仅接受 HTTPS 远程网关地址的 Expo 原生外壳。
...
桌面外壳可以立即监管并包含 DSH 运行时，而移动应用在认证网关存在前不能连接。Apple 签名、公证、TestFlight 配置和原生计算机使用服务仍属于发布与平台工作，不会隐藏在 Web 运行时中。
