...
The fork contains two private product workspaces under `apps/`. `apps/desktop` starts a local DSH web runtime and renders only its loopback URL in an Electron window with Node integration disabled. Its private `apps/desktop-runtime` deploy root supplies a symlink-free built DSH closure under the packaged app's `Resources/dsh-runtime` directory. The desktop main process launches that fixed entry through Electron Node mode, assigns `DSH_HOME` below Electron user data, and ignores `DSH_DESKTOP_RUNTIME` in a packaged app. The host restores validated window placement only when it intersects a current display work area, retains normal bounds beneath a maximized window, waits for the local page before showing the native window, and exposes only standard native menus for browser focus controls. `apps/mobile` is an Expo native shell that accepts only an HTTPS remote-gateway address.
